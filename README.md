...
退避された各値は、pickle バイト列の SHA-256 ハッシュが退避 ID に埋め込まれて保存されます。取得時にハッシュを検証します：

- 保存後にデータベースの行が改ざんまたは破損していた場合、即座に
  `RuntimeError("hash mismatch")` が発生します。
- SQLite の破損や外部からの値の改ざんを **検出** します。
